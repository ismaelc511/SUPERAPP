...
import superapp.models.reportesciudadanos.OcupacionIrregular;
...
public class OcupacionIrregularTask implements Task {

    OcupacionIrregular ocupacionIrregular;

    public OcupacionIrregularTask (OcupacionIrregular ocupacionIrregular){this.ocupacionIrregular = ocupacionIrregular;}
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                WaitElement.isClickable(OCUPACION_IRREGULAR),
                Click.on(OCUPACION_IRREGULAR),
                WaitElement.isClickable(DIRECCION_DAÑO),
                Enter.theValue(ocupacionIrregular.getDireccion()).into(DIRECCION_DAÑO),
                WaitFor.aTime(2000),
                WaitElement.isClickable(DIRECCION_ELEGIDA),
                Click.on(DIRECCION_ELEGIDA),
                WaitElement.isClickable(PUNTO_REFERENCIA),
                Enter.theValue(ocupacionIrregular.getPuntoReferencia()).into(PUNTO_REFERENCIA),
                ...
                ScrollAndClick.scrollAndClick(ocupacionIrregular.getDescripcion()),
                Click.on(ACEPTAR_TIPO_DAÑO_ABEJAS),
                WaitElement.isClickable(CORREO_ABEJAS),
                Click.on(CORREO_ABEJAS),
                Enter.theValue(ocupacionIrregular.getCorreoContacto()).into(CORREO_ABEJAS),
                WaitElement.isClickable(CONTINUAR_ABEJAS),
                Click.on(CONTINUAR_ABEJAS)
        );
    }

    public static OcupacionIrregularTask ocupacionIrregularTask(OcupacionIrregular ocupacionIrregular){
        return instrumented(OcupacionIrregularTask.class, ocupacionIrregular);
    }
}
